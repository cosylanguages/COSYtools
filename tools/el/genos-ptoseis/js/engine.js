class GreekGenderCasesEngine {
    ...
    getEndingClass(noun, data) {
        if (data.irregular) return { key: 'exception', label: 'Εξαίρεση: απομνημόνευση' };
        const plain = noun.normalize('NFD').replace(/[\u0300-\u036f]/g, '').toLowerCase();
        const gender = data.gender;
        if (gender === 'Αρσενικό' && ['ος', 'ης', 'ας'].some(suffix => plain.endsWith(suffix))) return { key: 'masculine', label: 'Αρσενικό: τυπική κατάληξη -ος/-ης/-ας' };
        if (gender === 'Θηλυκό' && ['α', 'η', 'ση', 'ξη', 'ψη', 'ωση'].some(suffix => plain.endsWith(suffix))) return { key: 'feminine', label: 'Θηλυκό: τυπική κατάληξη -α/-η' };
        if (gender === 'Ουδέτερο' && ['ο', 'ι', 'μα'].some(suffix => plain.endsWith(suffix))) return { key: 'neuter', label: 'Ουδέτερο: τυπική κατάληξη -ο/-ι/-μα' };
        return { key: 'ambiguous', label: 'Αμφίβολη κατάληξη: μάθε το άρθρο' };
    }
    ...
    nextEndingQuestion() {
        ...
        const labels = ['Τυπικά αρσενική', 'Τυπικά θηλυκή', 'Τυπικά ουδέτερη', 'Αμφίβολη κατάληξη', 'Εξαίρεση'];
        ...
        if (label) label.textContent = 'Ποια κλάση υποδηλώνει η κατάληξη;';
    ...
    checkEndingChoice(selected) {
        ...
        feedback.innerHTML = isCorrect
            ? `✅ Σωστά! ${expected.label}. Γένος: <strong>${data.gender}</strong>.`

...
class ConjugationEngine {
    ...
    nextIrregularQuestion() {
        ...
        const randomVerb = verbs[Math.floor(Math.random() * verbs.length)];
        const verbData = this.verbDb[randomVerb];
        const tenseKeys = ['indicatif_present', 'pc', 'indicatif_futur_simple', 'conditionnel_present', 'subjonctif_present'];
        const tenseNameMap = {
            indicatif_present: 'Présent',
            pc: 'Passé composé',
            indicatif_futur_simple: 'Futur simple',
        ...
        const availableTenses = tenseKeys.filter(key => Array.isArray(verbData.tenses?.[key]) && verbData.tenses[key].length > 0);
        ...
        const cleanExpected = targetForm
            .replace(/^((je|tu|il\/elle|nous|vous|ils\/elles|que|qu'|que tu|que nous|que vous)\s+|j')/i, '')
            .trim();
        const displayPronoun = targetForm.startsWith("j'") ? "j'" : (isSingleForm ? 'il' : (pronouns[randomIdx] || 'il/elle'));
        ...
        document.getElementById('game-verb-prompt').textContent = randomVerb;
        document.getElementById('game-tense-badge').textContent = tenseNameMap[randomTense] || 'Présent';
        document.getElementById('game-pronoun-prompt').textContent = `${this.irregularFamily(randomVerb)} · ${displayPronoun}`;
        gameAnsInput.value = '';
        gameAnsInput.disabled = false;
        gameAnsInput.focus();
        document.getElementById('game-feedback-box').style.display = 'none';
        document.getElementById('game-submit-btn').style.display = 'block';
        document.getElementById('game-next-btn').style.display = 'none';
    ...
    irregularFamily(verb) {
        if (/^(venir|tenir|prévenir|revenir|devenir|retenir|obtenir)/.test(verb)) return 'Famille venir/tenir';
        if (/^(prendre|apprendre|comprendre|reprendre|surprendre)/.test(verb)) return 'Famille prendre';
        if (/^(mettre|admettre|permettre|promettre|transmettre)/.test(verb)) return 'Famille mettre';
        if (verb.endsWith('uire')) return 'Famille -uire';
        if (verb === 'aller') return 'Suppletif: aller / vais / allé';
        if (verb === 'faire') return 'Famille faire';
        if (/^(pouvoir|vouloir|devoir|savoir|boire|croire|voir|lire)/.test(verb)) return 'Alternance vocalique';
        return 'Famille irrégulière';
    }

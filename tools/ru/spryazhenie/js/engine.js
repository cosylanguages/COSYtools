class RussianConjugationEngine {
    constructor() {
        ...
        this.srsStore = typeof SpacedRepetitionStore !== 'undefined' ? new SpacedRepetitionStore('cosy-ru-spryazhenie') : null;
    ...
    nextIrregularQuestion() {
        ...
        const tenseKey = Math.random() < 0.5 ? 'pres' : 'past';
        const forms = data.tenses[tenseKey] || [];
        const pIdx = Math.floor(Math.random() * forms.length);
        const pronouns = tenseKey === 'past' ? ['он', 'она', 'оно', 'они'] : ['я', 'ты', 'он/она', 'мы', 'вы', 'они'];
        const target = (forms[pIdx] || '')
            .replace(/[\u0300-\u036f]/g, '')
            .replace(/^(я|ты|он\/она́|он|она́|она|оно́|оно|мы|вы|они́|они)\s+/i, '')
            .trim();

        this.currentQuestion = { verb, pronoun: pronouns[pIdx] || 'они', expected: target };
        document.getElementById('game-verb-prompt').textContent = verb;
        document.getElementById('game-tense-badge').textContent = tenseKey === 'pres' ? 'Настоящее время' : 'Прошедшее время';
        document.getElementById('game-pronoun-prompt').textContent = `${this.irregularFamily(data)} · ${this.currentQuestion.pronoun}`;
        document.getElementById('game-feedback-box').style.display = 'none';
        document.getElementById('game-next-btn').style.display = 'none';
    ...
    irregularFamily(data) {
        if (data.group === 'разносклоняемое') return `Разносклоняемый · вид: ${data.pair || '—'}`;
        if (data.group && data.group.includes('искл.')) return `Исключение спряжения · вид: ${data.pair || '—'}`;
        return `Иррегулярный глагол · вид: ${data.pair || '—'}`;
    }

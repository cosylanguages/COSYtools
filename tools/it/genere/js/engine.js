class ItalianGenderEngine {
    ...
    nextGameQuestion() {
        ...
        if (label) label.textContent = 'Qual è il genere di:';
    ...
    getEndingClass(noun, data) {
        if (data.irregular) return { key: 'exception', label: 'Eccezione da memorizzare' };
        const endingRules = [
            { key: 'feminine', suffixes: ['zione', 'sione', 'ità', 'ezza', 'anza', 'enza', 'ice'], label: 'Desinenza tipicamente femminile' },
            { key: 'masculine', suffixes: ['mento', 'ore', 'one', 'ismo', 'aggio'], label: 'Desinenza tipicamente maschile' }
        ];
        const rule = endingRules.find(candidate => candidate.suffixes.some(suffix => noun.endsWith(suffix)));
        if (rule) return rule;
        if (noun.endsWith('a')) return { key: 'feminine', label: 'Finale -a: tipicamente femminile' };
        if (noun.endsWith('o')) return { key: 'masculine', label: 'Finale -o: tipicamente maschile' };
        return { key: 'ambiguous', label: 'Desinenza ambigua: imparare con l’articolo' };
    }
    ...
    nextEndingQuestion() {
        ...
        const keys = ['feminine', 'masculine', 'ambiguous', 'exception'];
        const labels = ['Desinenza tipicamente femminile', 'Desinenza tipicamente maschile', 'Desinenza ambigua', 'Eccezione da memorizzare'];
        this.currentQuestion = { noun, expectedPattern: pattern.key };
        document.getElementById('game-noun-prompt').textContent = noun;
        const label = document.querySelector('.prompt-label');
        if (label) label.textContent = 'Quale classe suggerisce la desinenza?';
    ...
    checkEndingChoice(selected) {
        ...
        feedback.innerHTML = isCorrect
            ? `✅ Corretto! ${expected.label}. Genere: <strong>${data.gender}</strong>.`

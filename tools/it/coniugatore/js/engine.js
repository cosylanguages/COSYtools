class ItalianConjugationEngine {
    ...
    setPracticeMode(mode) {
        ...
        const iBtn = document.getElementById('mode-irregular-btn');
        ...
        if (iBtn) iBtn.className = mode === 'irregular' ? 'badge active-mode' : 'badge';
    ...
    nextGameQuestion() {
        if (this.gamePracticeMode === 'irregular') {
            this.nextIrregularQuestion();
            return;
        }
    ...
    nextIrregularQuestion() {
        ...
        const verbs = Object.keys(this.verbDb).filter(verb => this.verbDb[verb].irregular === true);
        if (verbs.length === 0) return;

        const verb = verbs[Math.floor(Math.random() * verbs.length)];
        const data = this.verbDb[verb];
        const tenseOptions = [
            { key: 'pres', label: 'Presente' },
            { key: 'fut', label: 'Futuro' },
            { key: 'part', label: 'Participio passato' }
        ].filter(option => Array.isArray(data.tenses[option.key]) && data.tenses[option.key].length > 0);
        const tense = tenseOptions[Math.floor(Math.random() * tenseOptions.length)];
        const forms = data.tenses[tense.key];
        const pIdx = Math.floor(Math.random() * forms.length);
        const pronouns = ['io', 'tu', 'lui/lei', 'noi', 'voi', 'loro'];
        const rawTarget = forms[pIdx];
        const target = rawTarget.replace(/^(io|tu|lui\/lei|noi|voi|loro|che io|che tu|che lui\/lei|che noi|che voi|che loro)\s+/i, '').trim();

        this.currentQuestion = { verb, pronoun: tense.key === 'part' ? 'participio' : pronouns[pIdx], expected: target };
        document.getElementById('game-verb-prompt').textContent = verb;
        document.getElementById('game-tense-badge').textContent = tense.label;
        document.getElementById('game-pronoun-prompt').textContent = `${this.irregularFamily(verb)} · ${this.currentQuestion.pronoun}`;
        document.getElementById('game-feedback-box').style.display = 'none';
        document.getElementById('game-next-btn').style.display = 'none';
    ...
    irregularFamily(verb) {
        const families = {
            essere: 'Suppletivo: sono / fui / stato',
            avere: 'Radice alternante: ho / ebbi / avuto',
            andare: 'Radice alternante: vado / andato',
            venire: 'Radice alternante: vengo / venuto',
            fare: 'Radice suppletiva: faccio / fatto',
            dire: 'Participio irregolare: dico / detto',
            potere: 'Radice alternante: posso / potuto',
            volere: 'Radice alternante: voglio / voluto',
            sapere: 'Radice alternante: so / saputo'
        };
        return families[verb] || 'Famiglia irregolare';
    }
